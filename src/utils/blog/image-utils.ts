...
export const imageReplacer = (content: string, lazyLoad = false, postTitle: string) => {
	...
	if (lazyLoad) {
		content = content.replace(/<img/g, `<img loading="lazy" alt="${postTitle}-reference-image"`)
	} else {
		content = content.replace(/<img/g, `<img alt="${postTitle}-reference-image"`)
	}
